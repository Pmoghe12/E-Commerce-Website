...
export class ProductDetailsComponent implements OnInit {
  ...
  constructor( private route: ActivatedRoute,
    ...
    addTocart()
    {
      //console.log(this.anuj._id);
      cart :Cart;
      const qty = this.ingre_qty.nativeElement.value; // to get value from qty dropdown

      // let id = this.products._id;
      let product_name = this.anuj.name; // sunglasses
      let product_count = qty;  //  4
      let product_image = this.anuj.productImagePath;  // uploades/sunglases.jpg
      let product_price = this.anuj.price; // 700
      let product_total = qty * product_price;  // 3200

      //console.log(this.anuj.name);
      //console.log(qty);
     
      const cartItems = new Cart(this.anuj._id,product_name,product_count,product_image,product_price,product_total);
